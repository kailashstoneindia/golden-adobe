...
2. **Delivery & Order Architecture:**
   ...
   - **Open-Box Verification**: The completion of a delivery requires the driver to input the OTP _and_ upload a proof-of-delivery photograph to our storage buckets (e.g., AWS S3).
   - **Permissions**: Ensure strict Role-Based Access Control (RBAC) so that _only_ the primary Artisan account can trigger order validations.
